...
export class AuthService {
    // 令牌存储键名
    static TOKEN_STORAGE_KEY = 'ssh_terminal_token';
    static TOKEN_EXPIRY_KEY = 'ssh_terminal_token_expiry';
    
    // 当前令牌信息
    static _currentToken = null;
    static _tokenExpiry = null;
    static _connectionRetryCount = 0;
     ...
     * 
     * @param {Object} credentials - 凭据对象 {host, port, user, password}
     * @returns {Promise<Object>} 令牌响应对象 {token, expiresInSec, success}
     * @throws {Error} 令牌获取失败
     */
    static async getSessionToken(credentials) {
        try {
     ...
     * 
     * @returns {string|null} 有效的令牌，如果无有效令牌则返回null
     */
     ...
     * 检查令牌是否有效且未过期。
     * 
     * @param {string} [token] - 要检查的令牌，如果不提供则检查当前令牌
     * @returns {boolean} 如果令牌有效且未过期则返回true
     */
     ...
     * 获取令牌的剩余有效时间（秒）。
     * 
     * @returns {number} 剩余秒数，如果令牌无效则返回0
     */
     ...
     * 
     * @returns {Object|null} 连接头对象，如果无有效令牌则返回null
     */
    static getConnectionHeaders() {
     ...
     * 处理连接重试逻辑。
     * 
     * @param {Object} credentials - 原始凭据（用于重新获取令牌）
     * @returns {Promise<Object|null>} 新的连接头，如果超出重试次数则返回null
     */
    static async handleConnectionRetry(credentials) {
     ...
     * @private
     */
    static _storeToken(token, expiryTime) {
        try {
